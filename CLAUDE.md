...
- **RGB:** 83 WS2812 per-key LEDs (no separate underglow) via PSPI MOSI on PB7, DMA ch4, ~6 MHz SPI clock. ZMK firmware drives them with the custom out-of-tree **rainy_rgb** engine (replaces ZMK underglow; 11 effects + spatial/reactive + functional indicators) — see [docs/rainy-rgb.md](docs/rainy-rgb.md)
...
- [docs/rainy-rgb.md](docs/rainy-rgb.md) — rainy_rgb out-of-tree lighting engine: 11 effects, XY calibration, functional indicators (CapsLock/Fn-highlight/battery), controls, build/flash
